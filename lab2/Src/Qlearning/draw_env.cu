...
unsigned int window_height = BOARD_SIZE*TILE_SIZE + 2 * PADDING;// +2 * MARGIN;
...
void boardinit()
{
	...
	int maxoff = MINE_COUNT;

	std::vector<int> rv_x = generate_data(MINE_COUNT + maxoff, 0, BOARD_SIZE - 1);
	std::vector<int> rv_y = generate_data(MINE_COUNT + maxoff, 0, BOARD_SIZE - 1);
	int i = 0;
