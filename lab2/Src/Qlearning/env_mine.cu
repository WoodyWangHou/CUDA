...
__global__ void clear_board(int2* state, int *board, int board_width, float* rewards);
__global__ void env_step(int2* cstate, int2* nstate, float* rewards, short* actions, int *board, int board_width, int board_height);
//////////////////////////////////////////////////////////////////
...
void env_mineCls::reset(int sid)
{
	...
	clear_board << <nblkInGrid, nthInBlk >> > (d_state[sid], d_board, m_board_width, d_reward);
...
void env_mineCls::step(int sid, short* actions)
{
	...
	env_step << <nblkInGrid, nthInBlk >> > (d_state[sid], d_state[sid^1], d_reward, actions, d_board, m_board_width, m_board_height);
}
...
__global__ void env_step(int2* cstate, int2* nstate,
	float* rewards, short* actions, int *board, int board_width, int board_height)
{
	int tid = threadIdx.x + blockIdx.x*blockDim.x;
